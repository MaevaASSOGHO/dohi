    ...
    <div className="grid gap-4">
      <div className="flex items-center gap-3">
        ...
        {status === "pending" && (
          <button
            ...
            disabled={busy}
            className="text-xs rounded border border-neutral-700 px-2 py-1 hover:bg-neutral-800 disabled:opacity-60"
          >
      ...
      {submission?.files && (
        <div className="mt-2 grid gap-3">
          <h4 className="text-sm font-medium">Derniers fichiers envoyés</h4>
      ...
      {status === "approved" && (
        <p className="text-sm text-emerald-300">Votre identité est vérifiée ✅.</p>
      )}
    </div>
  );
}
...
function StatusBadge({ status }) {
  const map = {
    unverified: { txt: "Non vérifié", cls: "text-neutral-300 border-neutral-700 bg-neutral-900/50" },
    pending:    { txt: "En attente de vérification", cls: "text-yellow-200 border-yellow-800 bg-yellow-900/30" },
    approved:   { txt: "Vérifié", cls: "text-emerald-300 border-emerald-800 bg-emerald-900/30" },
    verified:   { txt: "Vérifié", cls: "text-emerald-300 border-emerald-800 bg-emerald-900/30" }, // compat
    rejected:   { txt: "Refusé", cls: "text-red-200 border-red-800 bg-red-900/30" },
  };
...
function Field({ label, children }) {
  return (
    <div>
      ...
      {children}
      <p className="mt-1 text-xs text-neutral-400">Formats: JPG/PNG/WEBP/HEIC (max 10 Mo).</p>
    </div>
  );
}
...
function FileDrop({ name, file, setFile, previewUrl }) {
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className="rounded border border-dashed border-neutral-700 p-3"
    >
      <div className="flex items-center gap-3">
        <div className="h-16 w-24 overflow-hidden rounded bg-neutral-900 border border-neutral-800 grid place-items-center">
          {previewUrl ? (
            <img
              src={previewUrl}
              alt=""
        ...
        <div className="flex-1">
          ...
          {file && <span className="ml-2 text-xs text-neutral-400">{file.name}</span>}
        </div>
      </div>
    </div>
  );
...
function ThumbBox({ label, url, fallbackName }) {
  return (
    <div className="w-28">
      <div className="h-20 w-28 overflow-hidden rounded border border-neutral-800 bg-neutral-900 grid place-items-center">
        {url ? (
          <img
            src={url}
            alt={label}
      ...
      <div className="mt-1 text-[10px] text-neutral-400 truncate" title={fallbackName || ''}>
        {label}
      </div>
    </div>
  );
